  ...
  return {
    ...
    html: `<!doctype html><html lang="pt-BR"><meta name="viewport" content="width=device-width, initial-scale=1"><body style="margin:0;background:#eef3f8;font-family:Arial,sans-serif;color:#172b45">
      ...
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="width:100%;max-width:600px;background:#fff;border-radius:16px;overflow:hidden">
        <tr><td style="padding:24px;background:#071936;color:#fff">${logo ? '<img src="cid:protovia-logo" width="120" alt="Logo" style="display:block;background:white;border-radius:8px;margin-bottom:16px">' : ''}<span style="font-family:Georgia,serif;font-size:24px">${escaparHtml(organization?.nome || 'Protovia')}</span> <span style="color:#9ad8f3">Protocolos</span></td></tr>
        ...
        ${appUrl ? `<p style="margin-top:28px"><a href="${escaparHtml(appUrl)}" style="display:inline-block;background:#0d365c;color:#fff;text-decoration:none;padding:14px 22px;border-radius:8px;font-weight:bold">Abrir Protovia</a></p>` : ''}
    ...
    body: JSON.stringify({
      ...
      html: `
        <div style="font-family:Arial,sans-serif;color:#172b45;line-height:1.55">
          ...
          <ul>${documentos}</ul>
          <p style="color:#667085;font-size:12px">Mensagem enviada automaticamente pelo Protovia Protocolos após confirmação da entrega.</p>
        </div>
      `
    })
  });
    ...
    body: JSON.stringify({
      ...
      ...await identidadeEmail('Uma nova entrega foi adicionada', `
        <div style="font-family:Arial,sans-serif;color:#172b45;line-height:1.55;max-width:620px">
          ...
          <p><strong>Documentos:</strong></p>
          <ul>${documentos}</ul>
          <p style="color:#667085;font-size:12px">Abra o Protovia Protocolos para acompanhar e confirmar a entrega.</p>
        </div>
      `)
    })
  });
