...
const {readPolicy,deliveryEvidence}=require('./delivery-policy');
...
function mountPickups(app, {database, optional = false}) {
  ...
  async function dbReady() {
    ...
    if (!ready) ready = (async()=>{
      ...
      if (!(await db.all('PRAGMA table_info(retiradas)')).some(col=>col.name==='contexto_json')) {
        try { await db.run('ALTER TABLE retiradas ADD COLUMN contexto_json TEXT'); }
        catch(error) { if (!(await db.all('PRAGMA table_info(retiradas)')).some(col=>col.name==='contexto_json')) throw error; }
      }
  ...
  async function access(req, db) {
    ...
    if (!req.usuarioLogado?.id) invalid('Entre no sistema.',401);
  }
  ...
  app.get('/api/retiradas/cadastros', route(async(req,res,db)=>{
    await access(req,db);
    const empresas=await db.all('SELECT id,nome,box,endereco,numero,bairro,cidade,uf FROM clientes WHERE ativo=1 ORDER BY nome');
    const recentes=await db.all('SELECT empresa_id,MAX(solicitado_em) AS ultima FROM retiradas WHERE solicitante_id=? GROUP BY empresa_id ORDER BY ultima DESC LIMIT 5',req.usuarioLogado.id);
    const idsRecentes=new Set(recentes.map(item=>Number(item.empresa_id)));
    const filtroEntregadores=req.usuarioLogado.perfil==='admin' ? "perfil IN ('entregador','admin')" : "perfil = 'entregador'";
    res.json({empresas,empresas_recentes:empresas.filter(item=>idsRecentes.has(Number(item.id))),entregadores:await db.all(`SELECT id,nome FROM usuarios WHERE ativo=1 AND ${filtroEntregadores} ORDER BY nome`)});
  }));
  app.get('/api/retiradas', route(async(req,res,db)=>{
    await access(req,db);
    const manager = canReview(req.usuarioLogado);
    const rows = await db.all(`SELECT * FROM retiradas ${manager?'':'WHERE entregador_id=? OR solicitante_id=?'} ORDER BY empresa_nome,solicitado_em DESC`,...(manager?[]:[req.usuarioLogado.id,req.usuarioLogado.id]));
    res.json(rows.map(({documentos_json,conferencia_json,contexto_json,...row})=>{
      const context=JSON.parse(contexto_json || '{}');
      const {localizacao,...gps}=context.gps || {};
      return {...row,contexto:{...context,gps:context.gps?{...gps,...(req.usuarioLogado.perfil==='admin'?{localizacao}:{})}:null},documentos:JSON.parse(documentos_json),conferencia:conferencia_json?JSON.parse(conferencia_json):null};
    }));
  }));
  app.post('/api/retiradas', route(async(req,res,db)=>{
    await access(req,db);
    const company = await db.get('SELECT id,nome,box,endereco,numero,bairro,cidade,uf FROM clientes WHERE id=? AND ativo=1',Number(req.body.empresa_id)||0);
    const courier = await db.get("SELECT id,nome,perfil FROM usuarios WHERE id=? AND ativo=1 AND perfil IN ('entregador','admin')",Number(req.body.entregador_id)||0);
    if (!company || !courier) invalid('Selecione empresa e responsável pela retirada ativos.');
    if (courier.perfil==='admin' && req.usuarioLogado.perfil!=='admin') invalid('Somente administradores podem atribuir uma retirada a outro administrador.',403);
    ...
    await db.run(`INSERT INTO retiradas(id,empresa_id,empresa_nome,solicitante_id,solicitante_nome,entregador_id,entregador_nome,solicitado_em,estado,documentos_json,observacao,contexto_json) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)`,id,company.id,company.nome,user.id,user.nome,courier.id,courier.nome,new Date().toISOString(),'solicitada',JSON.stringify(docs),text(req.body.observacao || '',2000,false),JSON.stringify({empresa:company}));
    res.status(201).json({id});
  }));
  app.delete('/api/retiradas/:id', route(async(req,res,db)=>{
    await access(req,db);
    if (req.usuarioLogado.perfil!=='admin') invalid('Somente administradores podem excluir retiradas.',403);
    const result=await db.run('DELETE FROM retiradas WHERE id=?',req.params.id);
    if (Number(result.changes)!==1) invalid('Retirada não encontrada.',404);
    res.json({ok:true});
  }));
  app.put('/api/retiradas/:id/retirar', route(async(req,res,db)=>{
    ...
    if(row.estado!=='solicitada')invalid('Esta retirada já foi atualizada. Recarregue a lista.',409);
    let gps;try {gps=deliveryEvidence(await readPolicy(db),{localizacao_entrega:req.body?.localizacao_entrega,justificativa_sem_gps:req.body?.justificativa_sem_gps},req.usuarioLogado.id);}catch(error){invalid(error.message);}
    const context={...JSON.parse(row.contexto_json || '{}'),gps,coletado_por:req.usuarioLogado.nome};
    const result=await db.run("UPDATE retiradas SET estado='aguardando_conferencia',retirado_em=?,contexto_json=? WHERE id=? AND estado='solicitada'",new Date().toISOString(),JSON.stringify(context),row.id);
