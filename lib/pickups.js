...
function mountPickups(app, {database, optional = false}) {
  ...
  app.get('/api/retiradas/cadastros', route(async(req,res,db)=>{
    await access(req,db);
    res.json({empresas:await db.all('SELECT id,nome,box,endereco,numero,bairro,cidade,uf FROM clientes WHERE ativo=1 ORDER BY nome'), entregadores:await db.all("SELECT id,nome FROM usuarios WHERE ativo=1 AND perfil IN ('entregador','admin') ORDER BY nome")});
  }));
