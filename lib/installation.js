...
function validOrganization(body) {
  ...
  if (!['grande', 'compacto'].includes(logoTamanho)) throw new Error('Escolha um tamanho válido para a marca.');
  return { nome, logo, corPrimaria, corSecundaria, logoTamanho };
}
...
function mountInstallation(app, { database, transaction, exigirLogin, exigirAdmin }) {
  ...
  app.get('/api/installation', async (_req, res) => {
    ...
    try {
      ...
      const organization = await db.get('SELECT nome, logo, cor_primaria, cor_secundaria, logo_tamanho FROM organizacao WHERE id = 1');
  ...
  app.post('/api/installation', async (req, res) => {
    ...
    try {
      await transaction(async db => {
        ...
        await db.run('INSERT INTO organizacao (id, nome, logo, cor_primaria, cor_secundaria, logo_tamanho) VALUES (1, ?, ?, ?, ?, ?)', organization.nome, organization.logo, organization.corPrimaria, organization.corSecundaria, organization.logoTamanho);
  ...
  app.put('/api/organization', exigirLogin, exigirAdmin, async (req, res) => {
    try {
      ...
      const db = await database();
      await ensureOrganizationSchema(db);
      await db.run('UPDATE organizacao SET nome = ?, logo = ?, cor_primaria = ?, cor_secundaria = ?, logo_tamanho = ? WHERE id = 1', organization.nome, organization.logo, organization.corPrimaria, organization.corSecundaria, organization.logoTamanho);
      res.json({ ok: true });
