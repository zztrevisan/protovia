...
test('instalação protegida, login, identidade e emissão com banco vazio', {timeout:25000}, async t => {
  ...
  const setup={organizacao:'Organização Teste',nome:'Administrador Teste',usuario:'gestor',senha:'Senha-de-teste-123!',token};
  ...
  assert.equal((await request('/api/organization','PUT',{organizacao:'Nova Marca',logo:''})).status,200);
  assert.equal((await request('/api/installation')).data.nome,'Nova Marca');
  ...
  assert.equal((await request('/api/organization','PUT',{organizacao:'Não permitido',logo:''})).status,403);
});
